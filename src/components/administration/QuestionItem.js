...
//   const [category, setCategory] = useState('')
  ...
  const categoryNameEnglish = question?"Edit Question":"New Question"
    ...
    <Row>
      ...
      <Col lg="10" className="m-auto">
              <div style={{width: '100%', borderStyle: 'solid',borderColor: 'black', borderWidth: 2, borderRadius: 5, marginBottom: 5, display: 'flex'}}>
                ...
                { !questionIsOpen && <ArrowDownIcon onClick={() => setQuestionIsOpen(true)} className="mr-2 ml-auto my-auto" style={{height: 16, width: 16, display: 'inline-block'}}></ArrowDownIcon>}
              
              </div>
              { questionIsOpen &&
              <div style={{backgroundColor: '#F6F6F6',}}>
               <QuestionForm refreshQuestions={refresh} question={question} canSubmit={canSubmit} setFinishedSaving={() => setFinished()} ideaStage={ideaStage} changeStatus={(status) => changeBtnStatus(status)} changeContinueStatus={(status) => changeContinueBtnStatus(status)} changeIdeaStage={() => this.changeIdeaStage()}/>
              </div>
              }
      </Col>
    </Row>
