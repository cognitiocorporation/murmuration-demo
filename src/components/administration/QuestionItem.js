...
function QuestionItem({user, updateQuestions}) {
...
//   const [category, setCategory] = useState('')
  ...
  const categoryNameEnglish = "New Question"
    ...
    <Row>
      ...
      <Col lg="10" className="m-auto">
              ...
              <div style={{backgroundColor: '#F6F6F6',}}>
               <QuestionForm refreshQuestions={refresh} userData={user} canSubmit={canSubmit} setFinishedSaving={() => setFinished()} ideaStage={ideaStage} changeStatus={(status) => changeBtnStatus(status)} changeContinueStatus={(status) => changeContinueBtnStatus(status)} changeIdeaStage={() => this.changeIdeaStage()}/>
              </div>
              }
      </Col>
    </Row>
